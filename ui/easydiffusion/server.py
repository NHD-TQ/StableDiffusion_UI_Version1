...
import io
server_api = FastAPI()
@server_api.post("/upload-image/")
async def load_image_from_request(file: UploadFile = File(...), device: str = "cpu"):


    raw_image = Image.open(io.BytesIO(await file.read())).convert('RGB')
    # raw_image = Image.open("/home/ducnh/Desktop/web_ui/easydiffusion/scripts/ca.jpg")
    images = raw_image
...
# Select model image2text
from fastapi import FastAPI, Form
@server_api.post("/process_tile")
async def processpro_tile(tiling: str = Form(...)):
    print("Lựa chọn tiling:", tiling)
    # Đường dẫn đến tệp văn bản bạn muốn ghi vào
    file_path = "/home/ducnh/Desktop/best_ui/easydiffusion/select_model.txt"  # Thay đổi đường dẫn tới tệp của bạn
    tiling = str(tiling)
    # Mở tệp với quyền ghi
    with open(file_path, "w") as file:
        # Ghi nội dung của biến tiling vào tệp
        file.write(tiling)
...
def init():
    ...
    @server_api.post('/uploadtext')
    async def generate_caption(filename: UploadFile = File(...)):
        max_length = 16
        num_beams = 4
        gen_kwargs = {"max_length": max_length, "num_beams": num_beams}

        file_path = "/home/ducnh/Desktop/best_ui/easydiffusion/select_model.txt"
        ...
        if model == "Salesforce/blip-image-captioning-large":
            ...
            device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
            # await load_image_from_request(filename, device=device)
            raw_image = await load_image_from_request(filename, device=device)
        ...
        # image = await load_image_from_request(filename, image_size=image_size, device=device)
        ...
        # caption = caption[0]  # Giả sử caption đã được tính toán từ đoạn mã xử lý chú thích
